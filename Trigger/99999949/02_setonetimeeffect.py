...
class Wait(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_onetime_effect(id=1, path='BG/Common/ScreenMask/Eff_CameraMasking_black.xml')
...
class Guide(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.debug_string(value='2번 영역에 들어가면 SetOnetimeEffect 트리거가 발동됩니다.')
...
class Quit(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_onetime_effect(id=1, path='BG/Common/ScreenMask/Eff_CameraMasking_black.xml')
        self.set_onetime_effect(id=4, path='BG/Common/Eff_Com_Vibrate_Short.xml')
        self.debug_string(value='7초 후에 트리거가 리셋됩니다. 2번 영역 밖으로 나가세요.')
