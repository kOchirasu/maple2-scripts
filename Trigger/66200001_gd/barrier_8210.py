...
class CheckSameUserTag7212(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(box_id=9210) and self.count_users(box_id=9210) == 2:
            return Enable7212(self.ctx)
        if self.count_users(box_id=9210) != 2:
            return Sensor7212(self.ctx)
        if not self.check_same_user_tag(box_id=9210):
            return SafeGreen7212(self.ctx)
        if self.user_value(key='Barrier21') == 10:
            return Reset(self.ctx)
...
class Enable7212(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10001189], state=1) # On

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10001189], state=0):
            # On
...
class Delay7212(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[10001205], state=1) # Off
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_value(key='Barrier21') == 10:
            return Reset(self.ctx)
        if self.object_interacted(interact_ids=[10001205], state=0):
            # Off
...
class CheckSameUserTag7213(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(box_id=9210) and self.count_users(box_id=9210) == 3:
...
class CheckSameUserTag7214(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(box_id=9210) and self.count_users(box_id=9210) == 4:
...
class CheckSameUserTag7215(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(box_id=9210) and self.count_users(box_id=9210) == 5:
