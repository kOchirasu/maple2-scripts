...
class ClearRoundDelay(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=2000):
            ...
            self.set_event_ui_script(type=BannerType.Bonus, script='$83000003_COLOSSEUM__ROUND10__8$', duration=3000)
...
class FailRoundDelay(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            self.set_event_ui_script(type=BannerType.Success, script='$83000003_COLOSSEUM__ROUND10__9$', duration=3000)
            return FailRound(self.ctx)
