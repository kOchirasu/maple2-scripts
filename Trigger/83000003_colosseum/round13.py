...
class 라운드조건체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_round() == 13:
            ...
            # self.set_event_ui_script(type=BannerType.Text, script='전투에서 승리하셨습니다. 이제 마지막 전투입니다. 끝까지 힘내십시오.', duration=3000)
...
class ClearRoundDelay(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=2000):
            ...
            self.set_event_ui_script(type=BannerType.Winner, script='$83000003_COLOSSEUM__ROUND13__8$', duration=3000)
...
class FailRoundDelay(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            self.set_event_ui_script(type=BannerType.GameOver, script='$83000003_COLOSSEUM__ROUND13__9$', duration=3000)
            return FailRound(self.ctx)
