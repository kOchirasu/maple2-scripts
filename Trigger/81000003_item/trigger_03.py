...
class 입장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_timer(timer_id='59', seconds=180, auto_remove=True, display=True, v_offset=-90, desc='wait.xml 시작 타이머 설정 UI') # 3분 후 시작
...
class 시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_timer(timer_id='160', seconds=160, display=True, desc='trigger_03.xml 시작 타이머 설정')
...
class 경기종료(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=6000):
            self.end_mini_game_round(winner_box_id=401, exp_rate=0.1333, is_gain_loser_bonus=True, game_name='UserMassive_Crazyrunner') # 승리자 및 실패자도 경험치 지급함
            self.end_mini_game(winner_box_id=401, game_name='UserMassive_Crazyrunner')
