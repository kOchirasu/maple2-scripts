""" trigger/51000003_dg/item_710.xml """
import trigger_api
...
class Spawn_check(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_effect(trigger_ids=[7710])
        self.set_effect(trigger_ids=[7720])

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='item_710_spawn') == 1:
            return SpawnItem(self.ctx)


class SpawnItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_effect(trigger_ids=[7710], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[720]):
            # 플레이어가 닿으면 획득
            return GetItem_Random(self.ctx)
        if self.user_value(key='item_710_spawn') == 0:
...
class GetItem_Random(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_effect(trigger_ids=[7710])
        self.set_effect(trigger_ids=[7720], visible=True)
...
class Buff_01(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_user_value(trigger_id=991119, key='item_710_spawn', value=0)
