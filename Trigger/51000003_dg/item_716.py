...
class Spawn_check(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_effect(trigger_ids=[7716])
        self.set_effect(trigger_ids=[7726])

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='item_716_spawn') == 1:
            return SpawnItem(self.ctx)


class SpawnItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_effect(trigger_ids=[7716], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[726]):
            # 플레이어가 닿으면 획득
            return GetItem_Random(self.ctx)
        if self.user_value(key='item_716_spawn') == 0:
...
class GetItem_Random(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_effect(trigger_ids=[7716])
        self.set_effect(trigger_ids=[7726], visible=True)
