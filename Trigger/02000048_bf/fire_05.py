...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[10000310], state=1)
        self.set_mesh(trigger_ids=[205])
        self.set_effect(trigger_ids=[305])

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000310], state=0):
...
class 오브젝트반응(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_mesh(trigger_ids=[205], visible=True, fade=1.0)
        self.set_effect(trigger_ids=[305], visible=True)
...
class NPC이동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_dialogue(type=1, spawn_id=405, script='$02000048_BF__FIRE_05__0$', time=2)
...
class 몬스터와전투(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[405]):
            return 딜레이(self.ctx)
        if not self.monster_in_combat(spawn_ids=[405]):
...
class 몬스터소멸(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_in_combat(spawn_ids=[405]):
