...
class Wait(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[10001167], state=2)
        self.destroy_monster(spawn_ids=[234]) # NPC
...
class NpcDown(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10001167], state=1)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='BattleEnd') == 1:
            return Quit(self.ctx)
        if self.object_interacted(interact_ids=[10001167], state=0):
...
class NpcWakeUp(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[10001167], state=2)
        self.spawn_monster(spawn_ids=[234], auto_target=False)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='BattleEnd') == 1:
            return Quit(self.ctx)
        if self.npc_detected(box_id=9900, spawn_ids=[224]):
...
class Quit(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[224,234])
        self.set_interact_object(trigger_ids=[10001167], state=0)
