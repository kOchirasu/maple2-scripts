""" trigger/52000037_qd/lookinto_berserker_04.xml """
import trigger_api
...
class Wait(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.quest_user_detected(box_ids=[9000], quest_ids=[60100070], quest_states=[3], job_code=20):
            # 초보자 넬프의 죽음 퀘스트 완료
            return Quit(self.ctx)
        if self.quest_user_detected(box_ids=[9900], quest_ids=[60100070], quest_states=[2], job_code=20):
            # 초보자 넬프의 죽음 퀘스트 완료 가능
            return Quit(self.ctx)
        if self.quest_user_detected(box_ids=[9900], quest_ids=[60100070], quest_states=[1], job_code=20):
            # 초보자 넬프의 죽음 퀘스트 수락
            return LoadingDelay02(self.ctx)
        if self.quest_user_detected(box_ids=[9000], quest_ids=[60100065], quest_states=[3], job_code=20):
            # 초보자 랄프의 정보 퀘스트 완료
            return DefaultSetting02(self.ctx)
        if self.quest_user_detected(box_ids=[9000], quest_ids=[60100065], quest_states=[2], job_code=20):
...
class Quit02(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_event_ui_script(type=BannerType.GameOver, script='$52000037_QD__LOOKINTO_BERSERKER_04__0$', duration=3000, box_ids='0')
