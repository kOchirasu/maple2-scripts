...
class 대기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.quest_user_detected(box_ids=[9000], quest_ids=[10003012], quest_states=[1], job_code=80):
            # 퀘스트 진행 중 상태
...
class 차전투대기1(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=7204, visible=True, initial_sequence='Down_Idle_B') # runebladerscout
...
class 미션완료01(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.quest_user_detected(box_ids=[9900], quest_ids=[10003012], quest_states=[2]):
            # 퀘스트 완료 가능 상태
