...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[10000296], state=1)
        self.set_actor(trigger_id=204, visible=True, initial_sequence='Dead_A')

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000296], state=0):
...
class NPC이동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=304, patrol_name='MS2PatrolData_204')
        self.set_dialogue(type=1, spawn_id=304, script='$02000046_AD__EAGLE_04__0$', time=2)
