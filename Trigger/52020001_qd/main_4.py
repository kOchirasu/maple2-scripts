...
class 알림(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_event_ui_script(type=BannerType.GameOver, script='출력이 부족해 크리티아스로 진입할 수 없습니다.', duration=4000)
...
class 알림_2(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_event_ui_script(type=BannerType.GameOver, script='적들을 처치하면 에너지를 충전할 수 있습니다.\\n제한시간 내에 100%충전해, 크리티아스로 진입하세요!', duration=4000)
