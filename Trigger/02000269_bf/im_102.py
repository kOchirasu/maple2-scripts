""" trigger/02000269_bf/im_102.xml """
import trigger_api


class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[10000677], state=1)
...
class 오브젝트반응(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000677], state=0):
            return 시간텀(self.ctx)
...
class NPC이동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_dialogue(type=1, spawn_id=1102, script='$02000269_BF__IM_102__0$', time=2)
