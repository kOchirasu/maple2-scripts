...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_event_ui_script(type=BannerType.GameOver, script='$02020063_BF__MESSAGE__0$', duration=5000)
...
class 체력공지_1(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_hp(spawn_id=801, is_relative=True) <= 50:
            # <게임 시작 결정>
            self.set_event_ui_script(type=BannerType.GameOver, script='$02020063_BF__MESSAGE__1$', duration=5000)
...
class 체력공지_2(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_hp(spawn_id=801, is_relative=True) <= 30:
            # <게임 시작 결정>
            self.set_event_ui_script(type=BannerType.GameOver, script='$02020063_BF__MESSAGE__2$', duration=5000)
...
class 체력공지_3(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_hp(spawn_id=801, is_relative=True) <= 10:
            # <게임 시작 결정>
            self.set_event_ui_script(type=BannerType.GameOver, script='$02020063_BF__MESSAGE__3$', duration=5000)
            return 종료(self.ctx)
