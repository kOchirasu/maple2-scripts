""" trigger/83000002_colosseum/round2.xml """
import trigger_api
...
class 대기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='StartRound2') >= 1:
...
class 라운드조건체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_round() == 2:
            self.side_npc_talk(npc_id=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND2__0$', duration=5000)
            # self.set_event_ui_script(type=BannerType.GameOver, script='전투에서 승리하셨습니다. 잠시만 기다려주세요 다음전투가 기다리고 있습니다!!', duration=3000)
            return 라운드대기(self.ctx)
        self.side_npc_talk(npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__1$', duration=3000)
...
class 라운드대기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.side_npc_cutin(illust='Startz_normal', duration=3000)
...
class 몬스터스폰(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(box_ids=[102], skill_id=69000501, level=1)
...
class 카운트(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.show_count_ui(text='$83000002_COLOSSEUM__ROUND2__2$', count=3, sound_type=2)
...
class 스폰대사(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.add_balloon_talk(spawn_id=102, msg='$83000002_COLOSSEUM__ROUND2__3$', duration=3000)
        self.set_timer(timer_id='LimitTimer', seconds=60, auto_remove=True)
        self.set_npc_duel_hp_bar(is_open=True, spawn_id=102, duration_tick=60000, npc_hp_step=10)

    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[102]):
            self.add_balloon_talk(spawn_id=102, msg='$83000002_COLOSSEUM__ROUND2__4$', duration=3000)
            self.set_npc_duel_hp_bar(spawn_id=102)
        ...
        if self.time_expired(timer_id='LimitTimer'):
            ...
            self.side_npc_talk(npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__5$', duration=3000)
        ...
        if self.user_detected(box_ids=[902]):
            ...
            self.side_npc_talk(npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__6$', duration=3000)
            self.destroy_monster(spawn_ids=[102])
            self.set_npc_duel_hp_bar(spawn_id=102)
            return FailRoundDelay(self.ctx)
        if not self.user_detected(box_ids=[904]):
            ...
            self.side_npc_talk(npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__7$', duration=3000)
            self.destroy_monster(spawn_ids=[102])
            self.set_npc_duel_hp_bar(spawn_id=102)
            return FailRoundDelay(self.ctx)
...
class ClearRoundDelay(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=2000):
            ...
            self.set_event_ui_script(type=BannerType.Bonus, script='$83000002_COLOSSEUM__ROUND2__8$', duration=3000)
...
class FailRoundDelay(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            ...
            self.set_event_ui_script(type=BannerType.Success, script='$83000002_COLOSSEUM__ROUND2__9$', duration=3000)
            return FailRound(self.ctx)
...
class ClearRound(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            ...
            self.side_npc_talk(npc_id=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND2__10$', duration=3000)
            self.set_user_value(trigger_id=900001, key='StartRound2', value=2)
...
class FailRound(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            self.set_user_value(trigger_id=900001, key='StartRound2', value=3)
            return 대기(self.ctx)
