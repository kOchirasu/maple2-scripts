""" trigger/02000213_bf/regenmob29.xml """
import trigger_api
...
class 소환몹등장(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000261], state=1):
            # arg2="0" 을 넣으면 몬스터가  등장하자마자 바로 공격 상태가 되는 것을 막을 수 있음
            self.spawn_monster(spawn_ids=[1029], auto_target=False)
            return 소멸체크(self.ctx)


class 소멸체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000261], state=0):
            return 소멸(self.ctx)
        if self.object_interacted(interact_ids=[10000261], state=2):
            return 소멸(self.ctx)
        if self.monster_dead(spawn_ids=[1029]):
