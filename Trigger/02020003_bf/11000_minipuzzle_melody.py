...
class Wait(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[12000202], state=2)
...
class EndMelodyQuiz_Success(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # RareBox 맵 별로 유니크하도록 변경해야 하는 값
        self.set_interact_object(trigger_ids=[12000202], state=1)
        self.set_timer(timer_id='2', seconds=60, auto_remove=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[12000202], state=0):
            # RareBox 맵 별로 유니크하도록 변경해야 하는 값
