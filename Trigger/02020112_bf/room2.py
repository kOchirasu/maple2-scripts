...
class 대기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_value(key='Extinction') >= 1 and self.user_value(key='Extinction_1_check') >= 1:
            return 시작(self.ctx)


class 시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_event_ui_script(type=BannerType.GameOver, script='$02020112_BF__ROOM2__0$', duration=3000)
...
class 격리(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_event_ui_script(type=BannerType.GameOver, script='$02020112_BF__ROOM2__1$', duration=5000, box_ids='932')
