""" trigger/02020112_bf/button8.xml """
import trigger_api
...
class 작동(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='ButtonSuccess') == 1:
            return 종료(self.ctx)
        if self.user_detected(box_ids=[924], job_code=0):
            return 감지(self.ctx)


class 감지(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=9908, visible=True, initial_sequence='Interaction_Lapentafoothold_A01_On')

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='ButtonSuccess') == 1:
            return 종료(self.ctx)
        if not self.user_detected(box_ids=[924], job_code=0):
            return 작동(self.ctx)
