...
class 티마이온등장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.side_npc_talk(npc_id=11004715, illust='Eone_serious', script='$52100303_QD__MAIN__0$', duration=3000)
        self.spawn_monster(spawn_ids=[101], auto_target=False)

    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[101]):
            self.set_achievement(type='trigger', achieve='KillTimaion')
...
class 종료대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.side_npc_talk(npc_id=11004715, illust='Eone_normal', script='$52100303_QD__MAIN__1$', duration=3000)
