...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_effect(trigger_ids=[6810])
        self.set_actor(trigger_id=210, visible=True, initial_sequence='Closed')
        self.spawn_monster(spawn_ids=[1010,1110], auto_target=False)
...
class 닭생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_effect(trigger_ids=[6810], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.time_expired(timer_id='1'):
            # self.set_dialogue(type=1, spawn_id=1010, script='후, 한결 낫네요', time=2)
            return 닭이동(self.ctx)
