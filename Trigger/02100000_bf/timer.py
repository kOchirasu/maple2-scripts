...
class 타이머시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_timer(timer_id='10000', seconds=360, auto_remove=True, display=True)
...
class 성공(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_event_ui_script(type=BannerType.Success, script='$02100000_BF__TIMER__1$', duration=2000, box_ids=['0'])
...
class 실패(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_event_ui_script(type=BannerType.GameOver, script='$02100000_BF__TIMER__0$', duration=2000, box_ids=['0'])
        self.destroy_monster(spawn_ids=[-1])
