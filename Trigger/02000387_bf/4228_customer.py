""" trigger/02000387_bf/4228_customer.xml """
import trigger_api
...
class CustomerEnter(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.spawn_monster(spawn_ids=[4228], auto_target=False)
...
class Patrol01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=4228, patrol_name='MS2PatrolData_401')
...
class Patrol02(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=4228, patrol_name='MS2PatrolData_402')
...
class Patrol03(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=4228, patrol_name='MS2PatrolData_403')
...
class PatrolEnd(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=9143, spawn_ids=[4228]):
            # 카운터 앞에 도착했으면
...
class PickItem_30000659(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_effect_nif(spawn_id=4228, nif_path='Map/Common/Indoor/co_in_prop_guestbook_A01.nif', is_outline=True, scale=1.2, rotate_z=225)
...
class RightItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.remove_effect_nif(spawn_id=4228)
        self.set_dialogue(type=1, spawn_id=4228, script='$02000387_BF__4228_CUSTOMER__0$', time=3)
...
class CustomerLeave(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=4228, patrol_name='MS2PatrolData_444')

    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=9304, spawn_ids=[4228]):
            return Quit(self.ctx)


class Quit(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[4228])
...
class WrongItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.remove_effect_nif(spawn_id=4228)
        self.set_dialogue(type=1, spawn_id=4228, script='$02000387_BF__4228_CUSTOMER__1$', time=3)
