""" trigger/02000387_bf/1213_customer.xml """
import trigger_api
...
class CustomerEnter(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.spawn_monster(spawn_ids=[1213], auto_target=False)
...
class Patrol01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=1213, patrol_name='MS2PatrolData_101')
...
class Patrol02(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=1213, patrol_name='MS2PatrolData_102')
...
class Patrol03(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=1213, patrol_name='MS2PatrolData_103')
...
class PatrolEnd(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=9113, spawn_ids=[1213]):
            # 카운터 앞에 도착했으면
...
class PickItem_30000691(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_effect_nif(spawn_id=1213, nif_path='Map/Tria/Indoor/tr_in_prop_paintbag_A01.nif', is_outline=True, scale=1.2, rotate_z=225)
...
class RightItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.remove_effect_nif(spawn_id=1213)
        self.set_dialogue(type=1, spawn_id=1213, script='$02000387_BF__1213_CUSTOMER__0$', time=3)
...
class CustomerLeave(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=1213, patrol_name='MS2PatrolData_111')

    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=9301, spawn_ids=[1213]):
            return Quit(self.ctx)


class Quit(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[1213])
...
class WrongItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.remove_effect_nif(spawn_id=1213)
        self.set_dialogue(type=1, spawn_id=1213, script='$02000387_BF__1213_CUSTOMER__1$', time=3)
