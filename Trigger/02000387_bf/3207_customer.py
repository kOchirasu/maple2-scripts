...
class CustomerEnter(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.spawn_monster(spawn_ids=[3207], auto_target=False)
...
class Patrol01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=3207, patrol_name='MS2PatrolData_301')
...
class Patrol02(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=3207, patrol_name='MS2PatrolData_302')
...
class Patrol03(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=3207, patrol_name='MS2PatrolData_303')
...
class PatrolEnd(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=9133, spawn_ids=[3207]):
            # 카운터 앞에 도착했으면
...
class PickItem_30000630(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_effect_nif(spawn_id=3207, nif_path='Map/Kerningcity/Indoor/ke_in_prop_display_C02.nif', is_outline=True, scale=1.2, rotate_z=225)
...
class RightItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.remove_effect_nif(spawn_id=3207)
        self.set_dialogue(type=1, spawn_id=3207, script='$02000387_BF__3207_CUSTOMER__0$', time=3)
...
class CustomerLeave(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.move_npc(spawn_id=3207, patrol_name='MS2PatrolData_333')

    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=9303, spawn_ids=[3207]):
            return Quit(self.ctx)


class Quit(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[3207])
...
class WrongItem(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.remove_effect_nif(spawn_id=3207)
        self.set_dialogue(type=1, spawn_id=3207, script='$02000387_BF__3207_CUSTOMER__1$', time=3)
