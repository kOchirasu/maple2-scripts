...
class 성공_3(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.add_buff(box_ids=[101], skill_id=50000230, level=1, ignore_player=False, is_skill_set=False)
...
class 실패(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.add_buff(box_ids=[101], skill_id=50000230, level=1, ignore_player=False, is_skill_set=False)
        self.set_event_ui_script(type=BannerType.GameOver, script='$02100009_BF__MAIN__0$', duration=2000, box_ids=['0'])
        self.destroy_monster(spawn_ids=[-1])
        self.set_portal(portal_id=3, visible=True, enable=True, minimap_visible=True)
