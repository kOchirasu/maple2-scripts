""" trigger/02020141_bf/interactmesh10014101.xml """
import trigger_api
...
class 최초시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10003149], state=2)
...
class 탈것_등장연출(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.spawn_monster(spawn_ids=[914101], auto_target=False)
...
class 탈것_등장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10003149], state=1)
        self.destroy_monster(spawn_ids=[914101])

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10003149], state=0):
...
class 인터렉트_동작중(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=40000):
            # 플레이어가 로봇탑승했으면 waitTick 이후 다시 리젠하기
            return 탈것_등장대기(self.ctx)
        if self.user_value(key='RidingBattle') == -1:
            return 종료(self.ctx)
