""" trigger/02020141_bf/mobspawn10.xml """
import trigger_api
...
class 트리거영역체크시작(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(spawn_ids=[99]):
            # 보스가 죽으면, 졸몹 등장 로직 정지
            return 졸몬스터제거작업(self.ctx)
        if self.user_detected(box_ids=[11000]):
            # MS2TriggerBox   TriggerObjectID = 11000, 이 트리거 박스 안에 플레이어가 한명이라도 체크 되면, 1시 지점에 설치된 트리거 박스
...
class 졸몬스터등장하기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.spawn_monster(spawn_ids=[11001,11002,11003,11004], auto_target=False)
...
class 트리거영역에계속있는지체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if not self.user_detected(box_ids=[11000]):
            # MS2TriggerBox   TriggerObjectID = 11000, 이 트리거 박스에 플레어어가 나가게 되면
...
class 졸몬스터제거작동대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[11000]):
            # MS2TriggerBox   TriggerObjectID = 11000 트리거 영역에 잠깐 벗어나 있다가 바로 들어오면, 이전 단계로 돌아가도록 하기
            return 트리거영역에계속있는지체크(self.ctx)
        if self.wait_tick(wait_tick=7000):
...
class 졸몬스터제거작업(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[11001,11002,11003,11004])
