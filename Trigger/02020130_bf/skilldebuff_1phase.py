...
class Ready(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        # SkillDebuffCheck_1Phase 변수 0으로 초기 세팅, 보스가 저주디버프스킬 사용할 때 이 트리거에거 이 변수 1 신호를 보냄
        self.set_user_value(key='SkillDebuffCheck_1Phase', value=0)
...
class 보스의저주디버프사용신호대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='SkillDebuffCheck_1Phase') == 1:
            # 보스가 플레이어에게 폭발 저주 디버프 부여 스킬 사용할 때 이 트리거에게  SkillDebuffCheck_1Phase = 1 신호를 보냄
        ...
        if self.user_value(key='FirstBattleEnd') == 99:
            # 이슈라가 1페이즈 끝내고 2페이즈 넘어갈 때 이 변수 99 보내서 저주 디버프 제거 단계 실행 후 이 트리거 작동 정지시킴
...
class 소환몹활성화될때까지잠시기다리기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5200):
            # !중요: 소환몹이 등장하여  전투 상태로 전환 되어 MonsterMany 변수에 1씩 더해지는 상황이 될때까지 이 단계에서 6초 정도 머물기, 5초 정도 해야 넉넉한 대기 시간임 혹시 3초 이하로 하면 넘 짧아서 베놈 스피릿이 공격 상태가 되기전에 트리거 단계가 넘어가 골치아픈 버그 생길 수 있으므로 넉넉히 5초 정도로 하자
...
class 폭발저주디버프제거잠시대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # 중요:  여기 단계 끝나면 다시 처음인 <state name="보스의저주디버프사용신호대기"> 단계로 넘어가는데, 거기서의  SkillDebuffCheck_1Phase =  1 진행되지 않도록 이 변수 꼭 0 초기화 하기
        self.set_user_value(key='SkillDebuffCheck_1Phase', value=0)
