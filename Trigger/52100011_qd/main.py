...
class QuestDungeonStart(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.quest_user_detected(box_ids=[702], quest_ids=[50100080], quest_states=[1]): # >
            return mermaid_01(self.ctx)
        if self.quest_user_detected(box_ids=[702], quest_ids=[50100080], quest_states=[2]):
...
class scene_02(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_dialogue(type=1, spawn_id=102, script='$52100011_QD__MAIN__5$', time=2, arg5=1)
...
class battle_03(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10002054], state=0):
