...
class 대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Cube') == 1:
            return 큐브17(self.ctx)


class 큐브17(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4029,4030,4031,4032], visible=True)
        self.set_mesh(trigger_ids=[5029,5030,5031,5032], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.set_user_value(trigger_id=920017, key='CubeOff', value=1)
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4029,4030,4031,4032])
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910017, key='Cube', value=0)
            return 종료(self.ctx)
