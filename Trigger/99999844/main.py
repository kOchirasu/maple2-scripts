...
class 기믹끝(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[905]):
            self.set_dialogue(type=1, spawn_id=105, script='웃기는군! 이동 할 수 있게 해줄게', time=4000)
            # self.add_balloon_talk(spawn_id=101, msg='웃기는군! 이동 할 수 있게 해줄게', duration=4000)
            return None # Missing State: 다리생성카운트
