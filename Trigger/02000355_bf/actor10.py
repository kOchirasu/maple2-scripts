...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_actor(trigger_id=210, visible=True, initial_sequence='Damg_B')
...
class 더미해제(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=210, initial_sequence='Damg_B')
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(spawn_ids=[2099]):
            return 소멸(self.ctx)
        if self.npc_detected(box_id=105, spawn_ids=[2010]):
...
class 리젠준비(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=210, visible=True, initial_sequence='Regen_A')
