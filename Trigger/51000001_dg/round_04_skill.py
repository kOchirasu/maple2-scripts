""" trigger/51000001_dg/round_04_skill.xml """
import trigger_api
...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[3402])
        self.set_mesh(trigger_ids=[3403])
        self.set_mesh(trigger_ids=[3404])
...
class 지역랜덤(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.random_condition(weight=25.0):
            ...
            return B지역(self.ctx)
        if self.random_condition(weight=25.0):
            self.set_mesh(trigger_ids=[3403], visible=True)
            return C지역(self.ctx)
        if self.random_condition(weight=25.0):
            self.set_mesh(trigger_ids=[3404], visible=True)
            return D지역(self.ctx)
...
class C지역(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[10403]):
            return 스킬랜덤(self.ctx)


class D지역(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[10404]):
            return 스킬랜덤(self.ctx)
