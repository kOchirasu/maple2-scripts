""" trigger/51000001_dg/round_08_skill.xml """
import trigger_api


class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[3801])
        self.set_mesh(trigger_ids=[3802])
        self.set_mesh(trigger_ids=[3803])
        self.set_mesh(trigger_ids=[3804])
        self.set_mesh(trigger_ids=[3805])
...
class 지역랜덤(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.random_condition(weight=20.0):
            self.set_mesh(trigger_ids=[3801], visible=True)
            return A지역(self.ctx)
        if self.random_condition(weight=20.0):
            self.set_mesh(trigger_ids=[3802], visible=True)
            return B지역(self.ctx)
        if self.random_condition(weight=20.0):
            self.set_mesh(trigger_ids=[3803], visible=True)
            return C지역(self.ctx)
        if self.random_condition(weight=20.0):
            self.set_mesh(trigger_ids=[3804], visible=True)
            return D지역(self.ctx)
        if self.random_condition(weight=20.0):
            self.set_mesh(trigger_ids=[3805], visible=True)
            return E지역(self.ctx)
