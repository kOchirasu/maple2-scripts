""" trigger/51000001_dg/round_02_skill.xml """
import trigger_api


class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[3201])
        self.set_mesh(trigger_ids=[3202])
        self.set_mesh(trigger_ids=[3203])
...
class 지역랜덤(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.random_condition(weight=34.0):
            self.set_mesh(trigger_ids=[3203], visible=True)
            return C지역(self.ctx)
...
class C지역(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[10203]):
            return 스킬랜덤(self.ctx)
