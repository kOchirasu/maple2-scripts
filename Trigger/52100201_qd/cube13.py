""" trigger/52100201_qd/cube13.xml """
import trigger_api
...
class 대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Cube') == 1:
            return 큐브13(self.ctx)


class 큐브13(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4022,4023,4024], visible=True)
        self.set_mesh(trigger_ids=[5022,5023,5024], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.set_user_value(trigger_id=920013, key='CubeOff', value=1)
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[5022,5023,5024])

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=4200):
            return 큐브13(self.ctx)
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910013, key='Cube', value=0)
            return 종료(self.ctx)
