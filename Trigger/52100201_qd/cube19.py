""" trigger/52100201_qd/cube19.xml """
import trigger_api
...
class 대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Cube') == 1:
            return 큐브19(self.ctx)


class 큐브19(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4035,4036,4037], visible=True)
        self.set_mesh(trigger_ids=[5035,5036,5037], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.set_user_value(trigger_id=920019, key='CubeOff', value=1)
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4035,4036,4037])
        self.set_mesh(trigger_ids=[5035,5036,5037])

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=4700):
            return 큐브19(self.ctx)
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910019, key='Cube', value=0)
            return 종료(self.ctx)
