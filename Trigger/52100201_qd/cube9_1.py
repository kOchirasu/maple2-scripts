""" trigger/52100201_qd/cube9_1.xml """
import trigger_api
...
class On_5(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[5003], visible=True)
...
class Off_7(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=200):
            self.set_user_value(trigger_id=920009, key='CubeOff', value=0)
            self.set_user_value(trigger_id=910009, key='Cube', value=2)
            return 대기(self.ctx)
