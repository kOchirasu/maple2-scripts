...
class 큐브9(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4014,4015,4016,4017], visible=True)
        self.set_mesh(trigger_ids=[5014,5015,5016,5017], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.set_user_value(trigger_id=920009, key='CubeOff', value=1)
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4014,4015,4016,4017])
        self.set_mesh(trigger_ids=[5014,5015,5016,5017])

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=4000):
            return 큐브9(self.ctx)
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910009, key='Cube', value=0)
            return 종료(self.ctx)
