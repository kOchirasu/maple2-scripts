...
class 큐브4(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4006,4007], visible=True)
        self.set_mesh(trigger_ids=[5006,5007], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.set_user_value(trigger_id=920004, key='CubeOff', value=1)
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4006,4007])
        self.set_mesh(trigger_ids=[5006,5007])

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3200):
            return 큐브4(self.ctx)
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910004, key='Cube', value=0)
            return 종료(self.ctx)
