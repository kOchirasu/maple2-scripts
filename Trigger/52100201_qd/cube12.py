""" trigger/52100201_qd/cube12.xml """
import trigger_api
...
class 대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Cube') == 1:
            return 큐브12(self.ctx)


class 큐브12(trigger_api.Trigger):
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4020,4021])

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3300):
            return 큐브12(self.ctx)
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910012, key='Cube', value=0)
            return 종료(self.ctx)
