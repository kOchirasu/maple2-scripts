""" trigger/52100201_qd/cube10.xml """
import trigger_api
...
class 대기(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Cube') == 1:
            return 큐브10(self.ctx)


class 큐브10(trigger_api.Trigger):
...
class 큐브제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[4018])

    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=2800):
            return 큐브10(self.ctx)
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910010, key='Cube', value=0)
            return 종료(self.ctx)
