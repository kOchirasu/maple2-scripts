""" trigger/52100201_qd/cube20.xml """
import trigger_api
...
class 큐브20(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=5000):
            self.set_user_value(trigger_id=920020, key='CubeOff', value=1)
...
class 큐브제거(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_detected(box_ids=[9004]):
            self.set_user_value(trigger_id=910020, key='Cube', value=0)
            return 종료(self.ctx)
