...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=1009, initial_sequence='Closed')

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[999996]):
            return 준비(self.ctx)
...
class 준비(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000531,10000522], state=0):
            return 트리거02시작(self.ctx)


class 트리거02시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=1009, visible=True, initial_sequence='Opened')
        self.spawn_monster(spawn_ids=[2036])
