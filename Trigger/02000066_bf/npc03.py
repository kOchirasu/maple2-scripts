...
class 시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10000343], state=0)
...
class 오브젝트반응(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10000343], state=1)

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000343], state=0):
            return 부활(self.ctx)
...
class NPC대사(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_actor(trigger_id=203, initial_sequence='Dead_A')
        self.set_dialogue(type=1, spawn_id=2003, script='$02000066_BF__NPC03__1$', time=2)
