...
class 안내(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.show_guide_summary(entity_id=1, text_id=26300734, duration=10)
...
class 몬스터체크(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        # 가이드 텍스트 ON : 슈퍼파워 핑크빈의 용병, 험상궂은 우르자들을 처치하세요!
        self.show_guide_summary(entity_id=1, text_id=26300734, duration=10000)
