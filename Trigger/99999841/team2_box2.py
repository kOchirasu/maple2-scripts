""" trigger/99999841/team2_box2.xml """
import trigger_api


class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_dungeon_variable(var_id=912, value=0)
...
class 시작(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.dungeon_variable(var_id=903) == 1:
            return 종료(self.ctx)
        if self.object_interacted(interact_ids=[10002180], state=0):
            return 애디셔널_중첩1(self.ctx)


class 애디셔널_중첩1(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_dungeon_variable(var_id=912, value=1)
