...
class 동전생성01(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[101]):
            ...
            self.set_event_ui_script(type=BannerType.Text, script='$99999896__02_COIN__0$', duration=2000)
...
class 동전생성02(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[102]):
            ...
            self.set_event_ui_script(type=BannerType.Text, script='$99999896__02_COIN__1$', duration=2000)
...
class 동전생성03(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[103]):
            self.set_event_ui_script(type=BannerType.Text, script='$99999896__02_COIN__2$', duration=2000)
            self.create_item(spawn_ids=[22,23,24,25,26,27,28,29,30])
            self.set_event_ui_script(type=BannerType.Winner, script='$99999896__02_COIN__3$', duration=2000)
