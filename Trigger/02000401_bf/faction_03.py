...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[12000027], state=2)
        self.set_interact_object(trigger_ids=[12000028], state=2)
...
class 탱크준비(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_dialogue(type=1, spawn_id=1201, script='$02000401_BF__FACTION_03__0$', time=5)
...
class 던전(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[12000027], state=1)
        self.set_interact_object(trigger_ids=[12000028], state=1)
...
class 퀘스트(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(trigger_ids=[12000039], state=1)
...
class 종료체크(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='DungeonClear') == 1:
            ...
            self.set_interact_object(trigger_ids=[12000039], state=0)
            self.remove_buff(box_id=199, skill_id=99910130)
            return 종료(self.ctx)
