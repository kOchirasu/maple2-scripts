...
class 지원군생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.move_npc(spawn_id=105, patrol_name='MS2PatrolData_15')

    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=3005, spawn_ids=[105]):
...
class 지원군이동(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(box_id=2001, spawn_ids=[105]):
...
class 지원군소멸(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_timer(timer_id='5', seconds=90)
