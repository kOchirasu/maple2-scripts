...
class 시작대기중(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10000612], state=0):
...
class NPC교체(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10000612], state=2)
...
class NPC이동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.move_npc(spawn_id=1091, patrol_name='MS2PatrolData_1091')
        self.set_effect(trigger_ids=[611], visible=True)
        self.set_dialogue(type=1, spawn_id=1091, script='$52000002_QD__SHEEP_01__0$', time=2)
...
class NPC소멸(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[1091])
