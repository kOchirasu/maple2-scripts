...
class NPC교체(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10000616], state=2)
        self.spawn_monster(spawn_ids=[1094])
...
class NPC이동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.move_npc(spawn_id=1094, patrol_name='MS2PatrolData_1094')
        self.set_effect(trigger_ids=[614], visible=True)
        self.set_dialogue(type=1, spawn_id=1094, script='$52000002_QD__SHEEP_04__0$', time=2)
...
class NPC소멸(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[1094])
