...
class 시작(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(box_ids=[1004]):
...
class 몬스터등장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.spawn_monster(spawn_ids=[161,162,163,164,165,166])

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='monster_die') == 1:
            return 몬스터소멸(self.ctx)
        if self.monster_dead(spawn_ids=[161,162,163,164,165,166]):
            return 대기(self.ctx)
...
class 몬스터소멸(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.destroy_monster(spawn_ids=[161,162,163,164,165,166])
